...
function Butt_Selecte_trip() {
  ...
  useEffect(() => {
    const fetchCityList = async () => {
      try {
        const response = await axios.get(`api/citylist/pickup/`)
        ...
        className='mt-8 group w-full flex flex-wrap  gap-4 lg:gap-0  lg:rounded-md lg:bg-color-canvas-primary lg:shadow-md'>
          ...
          />
            ...
            <div className='flex justify-start items-center px-3 gap-4 w-full h-14  ' >
              ...
              <InputField list={list} selected={selectfromCity} setSelected={setselectfromCity} />
              </div>
            </div>
            <div 
            ...
            <div className='relative flex justify-start items-center px-3 gap-4 w-full h-14 bsg-stone-200 '>
              ...
              <InputField list={list} selected={selecttoCity} setSelected={setselecttoCity} />
              </div>
            </div>
          ...
          <div className='w-full lg:w-7/12  flex flex-wrap gap-4 sm:gap-0 sm:rounded-lg sm:shadow-lg lg:shadow-none overflow-hidden sm:border bodrder-[#b7e4ff] lg:rounded-l-lg lg:rounded-r-none lg:border-r-0 '>
            <div className='w-full h-14 sm:w-10/12 md:w-9/12 lg:w-8/12 flex flex-wrap justify-start items-center gap-4 px-4 bg-slate-200 shadow-lg sm:shadow-none sm:flex-nowrap  rounded-lg sm:rounded-none'>
              <CalendarDaysIcon className='w-max h-full py-2 text-[#005687]'/>
              <h1>الخميس ,19, سبتمبر </h1>
              <button className='flex justify-center bg-[#f4fafe] h-max border  text-gray-500 border-[#23547011] px-3 py-0.5 rounded-xl '>
                اليوم
              </button>
              <button className='flex justify-center bg-blue-100 text-[#005687] font-bold h-max shadow-sm border bordser-[#437ea186] px-3 py-0.5 rounded-xl'>غداً</button>
            </div>
            <button
            type='submit'
  ...
  return (
    <div className='pt-6 pb-0 mb-0 px-2 w-full flex flex-col justify-center items-center'>
      <Tabs style={{margin:''}} aria-label="Options" classNames={{ tabContent: 'text-primary ' }} color='primary'   onSelectionChange={settogglebtn as any} fullWidth className='w-full max-w-xl ' disabledKeys={['3','2']}>
